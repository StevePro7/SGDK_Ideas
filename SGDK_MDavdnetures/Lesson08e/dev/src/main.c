...
void myJoyHandler( u16 joy, u16 changed, u16 state )
{
	...
	if( joy == JOY_1 )
	{
		...
		if( estado == 9 && ( changed & BUTTON_A ) )
		{
			...
			u16 *shadowmap_tilemap = fondo_doble_prioridad.tilemap->tilemap;
			...
			VDP_setTileMapDataRectEx( BG_A, &tilemap_buffer[ 0 ], 0/*u16 basetile*/,
				...
				SCENARIO_WIDTH_TILES, SCENARIO_HEIGHT_TILES, SCENARIO_WIDTH_TILES, NULL );
		...
		if( estado == 10 && ( changed & BUTTON_B ) )
		{
			...
			for( i = 0; i < NUM_LINES; i++ )
			{
			 ...
			 // Este comando le dice cuanto tiene que desplazar el scroll del plano A (BG_A),
