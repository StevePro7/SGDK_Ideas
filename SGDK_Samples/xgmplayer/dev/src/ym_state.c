...
static void YM_keyOFF(YM_SLOT *slot);
static void YM_keyON(YM_SLOT *slot);
...
void YM_XGMKeyWrites(YM *ym, u8* data, u16 num)
{
    ...
    while(num--)
    {
        ...
        if (value & 0x10) YM_keyON(&(ch->slots[0]));
        else YM_keyOFF(&(ch->slots[0]));
        if (value & 0x20) YM_keyON(&(ch->slots[1]));
        else YM_keyOFF(&(ch->slots[1]));
        if (value & 0x40) YM_keyON(&(ch->slots[2]));
        else YM_keyOFF(&(ch->slots[2]));
        if (value & 0x80) YM_keyON(&(ch->slots[3]));
        else YM_keyOFF(&(ch->slots[3]));
    }
}
