...
int main()
{
	...
	data3 = morris_sprite.palette->data;
	data4 = muerte_sprite.palette->data;
	numTile1 = fondo1.tileset->numTile;
	numTile2 = fondo2.tileset->numTile;
	numTile3 = fondo3.tileset->numTile;
	numTile4 = fondo_doble.tileset->numTile;
	numTile5 = fondo_doble_prioridad.tileset->numTile;
	tilemap = fondo_doble_prioridad.tilemap->tilemap;
#endif
...
void myJoyHandler( u16 joy, u16 changed, u16 state )
{
	if( joy == JOY_1 )
	{
		...
		if( estado == 9 && ( changed & BUTTON_A ) )
		{
			...
			VDP_clearPlane( BG_A, TRUE ); //adios fondo. TRUE espera a que termine
			VDP_clearPlane( BG_B, TRUE ); //de borrarlo
			...
			VDP_setTileMapDataRectEx( BG_A, &tilemap_buffer[ 0 ], 0/*u16 basetile*/,
				...
				SCENARIO_WIDTH_TILES, SCENARIO_HEIGHT_TILES, SCENARIO_WIDTH_TILES, DMA );
			ind += numTile5;
